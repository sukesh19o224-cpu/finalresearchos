...
export class PDFReportGenerator {
  ...
  private addTitlePage(metadata: ReportMetadata): void {
    ...
    this.doc.text('ResearchOS', centerX, 60, { align: 'center' })
  ...
  private addPageNumbers(): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Add timestamp
      this.doc.text(
        `Generated by ResearchOS on ${format(new Date(), 'yyyy-MM-dd HH:mm')}`,
        this.margin,
        this.pageHeight - 10
      )
    }
...
export async function generateQuickReport(
  ...
  sections.push({
    type: 'text',
    content:
      'This report contains the results of electrochemical experiments conducted using ResearchOS.',
  })
